...
print("==========================================================")
for key, value in test_results_05.items():  # .items() : 딕셔러니에 저장된 모든 키, 값 쌍을 한 번에 가져오는 역할
    # 딕셔너리 예시: results = {'정밀도': 0.95, '재현율': 0.88, 'F1': 0.91}
    # .items() 반환 값: dict_items([('정밀도', 0.95), ('재현율', 0.88), ('F1', 0.91)])
...
# 최적 임계값 선정
optimal_idx = np.argmax(fscores)    # F1-score 값 중에서 가장 큰 값(최댓값)의 인데스 위치를 찾음. 그 위치가 최적의 성능을 나타내는 지점
optimal_threshold = thresholds[optimal_idx] # optimal_idx를 사용하여, 원래의 임계값 배열에서 그 위치에 해당하는 임계값 추출(= F1-score를 최대로 만들어 주는 임계값의 위치를 찾아서 지정해준다.)
...
# 위음성, 위양성 사례를 저장할 빈 리스트를 만들어줌
false_negatives_list = []
false_positives_list = []
num_test_samples = len(X_test)  # 테스트 데이터셋에 포함된 전체 이미지의 개수를 계산하여 저장

# 모든 테스트 샘플을 순회하며 오류를 검사
for i in range(num_test_samples):  # 테스트 데이터셋의 전체 이미지 개수만큼 반복
    filepath = X_test[i] # 이미지 파일 경로
    label = int(test_labels[i]) # 실제 정답 레이블(0 또는 1)
    prob = test_probs[i]    # 모델이 예측한 이상(1)일 확률
