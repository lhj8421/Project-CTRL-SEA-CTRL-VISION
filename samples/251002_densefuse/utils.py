...
# +++++++++++++++++++++++++++++++++++++++++++
def list_image_pairs(directory):
    ir_images = {}
    rgb_images = {}
    image_pairs = []
    ...
    for file in os.listdir(directory):
        ...
        if filename.startswith('ir_'):
            key = filename[3:].split('.')[0]
            ir_images[key] = filepath
        elif filename.startswith('rgb_'):
            key = filename[4:].split('.')[0]
            rgb_images[key] = filepath

    # 쌍으로 있는 파일들만 매칭
    for key in ir_images:
        if key in rgb_images:
            image_pairs.append((ir_images[key], rgb_images[key]))
